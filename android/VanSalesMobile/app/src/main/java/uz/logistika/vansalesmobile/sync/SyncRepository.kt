...
class SyncRepository(
    ...
    fun login(baseUrl: String, database: String, login: String, password: String): JSONObject {
        ...
        session.login = login
        val result = MobileApiClient(session.baseUrl, database).login(database, login, password)
        session.token = result.getString("token")
        result.optJSONObject("bootstrap")?.let { db.applyBootstrap(it) }
        return result
    }

    fun bootstrap(): JSONObject {
        require(session.token.isNotBlank()) { "Login required" }
        val result = MobileApiClient(session.baseUrl, session.database).bootstrap(session.token)
        db.applyBootstrap(result)
        return result
    }
    ...
    fun syncPending(): JSONObject {
        ...
        val result = MobileApiClient(session.baseUrl, session.database).sync(session.token, pending)
    ...
    private fun client(): MobileApiClient {
        require(session.token.isNotBlank()) { "Login required" }
        return MobileApiClient(session.baseUrl, session.database)
    }
}
