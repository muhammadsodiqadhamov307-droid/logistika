...
class VanTripLine(models.Model):
    ...
    trip_date = fields.Datetime(related='trip_id.date', string='Sana', store=True, readonly=True)
    trip_agent_id = fields.Many2one('res.users', related='trip_id.agent_id', string='Agent', store=True, readonly=True)
    trip_taminotchi_id = fields.Many2one('van.taminotchi', related='trip_id.taminotchi_id', string='Taminotchi', store=True, readonly=True)
